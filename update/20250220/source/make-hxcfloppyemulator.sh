...
# https://github.com/jfdelnero/HxCFloppyEmulator/issues/32#issuecomment-2655627737
# build HxC itself by compiling FLTK without Wayland support
cd $HOME/source/HxCFloppyEmulator/HxCFloppyEmulator_software/sources/thirdpartylibs/fltk
chmod a+x prepare_fltk.sh
./prepare_fltk.sh

cd $HOME/source/HxCFloppyEmulator/HxCFloppyEmulator_software/sources/thirdpartylibs/fltk/fltk-1.x.x/
...
if [ $? -eq 0 ]
then
        echo "Compilation of FLTK libraries was successful."
        echo
        sudo make install
else
        echo "Compilation of FLTK libraries was NOT successful.  Aborting installation."
        echo
        exit 1
fi


cd $HOME/source/HxCFloppyEmulator/build
make -j$cores
