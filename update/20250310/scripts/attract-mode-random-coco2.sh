...
do
	...
	if [[ $ext == 'BAS' ]]; then
		...
		sleep 2
		mame coco2b -ram 64k -flop1 "$file" -seconds_to_run 120 -autoboot_delay 2 -autoboot_command "RUN \"$program\"\n" $MAMEPARMS
		BAS=1
	...
	if [[ $BAS != 1 ]]; then
		...
		if [[ $ext == 'BIN' ]]; then
			...
			echo Program to run/execute: $program
			sleep 2
			mame coco2b -ram 64k -flop1 "$file" -seconds_to_run 120 -autoboot_delay 2 -autoboot_command "LOADM \"$program\":EXEC\n" $MAMEPARMS
	...
	mame coco2b -ram 64k -cart "$file" -seconds_to_run 120 $MAMEPARMS
	...
	mame coco2b "$filename_no_ext" -ram 64k -seconds_to_run 120 $MAMEPARMS
