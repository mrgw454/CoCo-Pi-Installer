...
sudo apt -y install glances neofetch net-tools dos2unix screenfetch screen psmisc curl nala xterm wget xfsprogs rmlint fdupes timelimit duf
...
sudo apt -y install vbindiff colordiff dhex fluidsynth qsynth
...
unzip -o -j TTF-source-code-pro-2.042R-u_1.062R-i.zip -d $HOME/.fonts
fc-cache -f -v
...
# for samba
sudo apt -y install samba --force-confdef samba-common-bin smbclient cifs-utils
echo
echo
...
if [ $NONCOCOPI -eq 1 ] && [ $systemtype = amd64 ] && [ $WSL -eq 0 ] && [KVMQEMU -eq 1 ]; then
	...
	# for HP printers
	sudo apt -y install hplip printer-driver-hpcups
