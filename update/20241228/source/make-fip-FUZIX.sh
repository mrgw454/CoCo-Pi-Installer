...
# search for following lines and delete them
sed -i 's|make -C $(FUZIX_DIR) TARGET=coco3 SUBTARGET=fpga kernel||' $HOME/source/fip-FUZIX/Makefile
sed -i 's|cp $(FUZIX_DIR)/Kernel/fuzix.bin boot/fuzix-fpga.bin||' $HOME/source/fip-FUZIX/Makefile
sed -i 's|make -C $(FUZIX_DIR) TARGET=coco3 SUBTARGET=nano kernel||' $HOME/source/fip-FUZIX/Makefile
sed -i 's|cp $(FUZIX_DIR)/Kernel/fuzix.bin boot/fuzix-nano.bin||' $HOME/source/fip-FUZIX/Makefile
#sed -i 's|make -C $(FUZIX_DIR)/Kernel TARGET=coco3 clean||' $HOME/source/fip-FUZIX/Makefile
#sed -i 's|make -C $(FUZIX_DIR)/Kernel TARGET=coco3 clean||' $HOME/source/fip-FUZIX/Makefile
