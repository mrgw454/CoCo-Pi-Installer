...
sudo apt -y install glances net-tools dos2unix screenfetch screen psmisc curl nala xterm wget xfsprogs
...
sudo apt -y install xxd gcc libncurses5-dev libffi-dev libgl1-mesa-dev libx11-dev libxext-dev libxrender-dev libxrandr-dev libxpm-dev libtinfo6 libgpm-dev
...
# install fastfetch as current replacement for neofetch
...
# install NON CoCo-Pi specific packages (for arm64 environments only)
if [ $NONCOCOPI -eq 1 ] && [ $systemtype = arm64 ] && [ $WSL -eq 0 ] && [KVMQEMU -eq 0 ]; then
	...
	sudo apt-get update && sudo apt -y --allow-unauthenticated install --reinstall d-apt-keyring
	sudo apt-get update && sudo apt -y install ldc
